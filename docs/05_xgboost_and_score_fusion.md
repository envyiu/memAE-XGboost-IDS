...
- `max_depth=8`: cây đủ sâu để bắt tương tác feature nhưng vẫn hạn chế overfit.
...
`_predict_prob(model, X)` xử lý best iteration:
...
Điều này đảm bảo inference dùng đúng số cây tốt nhất khi early stopping hoạt động. Detector calibration và fusion dùng logic tương tự.
...
`_predict_prob_batched()` dự đoán theo batch 100,000 rows để tránh load/predict một mảng quá lớn cùng lúc.
...
`_fusion_features(xgb_score, memae_score)` tạo 6 chiều:
...
Logistic fusion học weight từ train labels. Threshold của fusion được chọn sau đó bằng benign calibration scores. Không được tune threshold trên `test_zero_day`.
